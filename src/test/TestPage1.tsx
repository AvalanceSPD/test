...
const TestPages1 = () => {
  ...
  const [selectedSubSessionId, setSelectedSubSessionId] = useState<
    number | null
  >(null);
  const [selectedSubSession, setSelectedSubSession] =
    useState<SubSession | null>(null);
    ...
    setSessions(
      sessions.map((session) => {
        if (session.id === sessionId) {
          return {
            ...session,
            subSessions: session.subSessions.filter(
              (sub) => sub.id !== subSessionId
            ),
          };
        }
        return session;
      ...
      <div className={styles.mainContent}>
        <div className={styles.leftSection}>
          ...
          {isMainContent ? (
            ...
            <>
              <div
                className={styles.videoContainer}
                onClick={() => selectedSubSession && setIsUrlModalOpen(true)}
              >
                {videoUrl ? (
                  <iframe
                    src={videoUrl}
                  ...
                  <div className={styles.uploadPlaceholder}>
                    {selectedSubSession
                      ? "+ Add Video URL"
                      : "เลือกบทเรียนย่อยเพื่อเพิ่มวิดีโอ"}
                  </div>
                )}
              </div>
              <textarea
                ...
                placeholder={
                  selectedSubSession
                    ? "Add description..."
                    : "เลือกบทเรียนย่อยเพื่อเพิ่มคำอธิบาย"
                }
                className={styles.descriptionInput}
        ...
        <div className={styles.rightSection}>
              ...
              <div key={session.id} className={styles.sessionItem}>
                ...
                <div className={styles.subSessionList}>
                  {session.subSessions.map((subSession) => (
                    <div
                      key={subSession.id}
                      className={`${styles.subSessionItem} ${
                        selectedSubSession?.id === subSession.id
                          ? styles.selected
                          : ""
                      }`}
                      onClick={() => {
          ...
          <div className={styles.navigationSection}>
            <div className={styles.sectionHeader}>
              <h4>Document</h4>
              <button onClick={() => openUrlModal("document")}>
                + Add Document
              </button>
            </div>
            {documents.map((doc) => (
